...
def convert_to_financial_data(board: List[List[str]], data_test: List[List[str]]) -> List[str]:
    """
    Convert MLP output and RSI test data to financial data.

    Args:
        board (List[List[str]]): MLP output data.
        data_test (List[List[str]]): RSI test data.

    Returns:
        List[str]: Converted financial data as a list of strings.
    """
    builder: List[str] = []
    ...
    for n in range(len(board)):
        if board[n][2].startswith("0.0"):
            counter_zeros += 1
        elif board[n][2].startswith("1.0"):
            counter_ones += 1
        elif board[n][2].startswith("2.0"):
            counter_twos += 1
