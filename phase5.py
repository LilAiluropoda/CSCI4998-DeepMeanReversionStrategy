...
class Backtester:
    ...
    def process_transactions(data: pd.DataFrame, company: str) -> TransactionStats:
        ...
        print(f"\nSignal distribution for {company}:")
        print(f"Buy signals (1.0): {signal_counts.get(1.0, 0)}")
        print(f"Sell signals (2.0): {signal_counts.get(2.0, 0)}")
        print(f"Hold signals (0.0): {signal_counts.get(0.0, 0)}\n")
        
        if signal_counts.get(1.0, 0) == 0 or signal_counts.get(2.0, 0) == 0:
            print(f"Warning: Missing buy or sell signals for {company}")
            return stats
        ...
        try:
            ...
            if stats.transaction_count == 0:
                print(f"Warning: No valid transactions completed for {company}")
                print("This might be due to:")
                print("1. No matching sell signals after buy signals")
                print("2. All transactions triggered stop-loss")
                print("3. Invalid signal sequence")
            
            return stats
            
        except Exception as e:
            print(f"Error in process_transactions: {e}")
            return stats
    ...
    def update_stats(stats: TransactionStats, gain: float, share_number: float, 
        ...
        try:
            if sell_point != 0:  # Prevent division by zero
...
class BackTestReportGenerator:
    ...
    def generate_report(stats: TransactionStats, money_bah: float, data_length: int, company: str) -> Dict[str, float]:
        ...
        try:
            number_of_years: float = max((data_length - 1) / 365, 1)  # Ensure at least 1 year
            
            # Calculate BaH metrics
            bah_return = money_bah - 10000.0
            bah_return_pct = (money_bah / 10000.0 - 1) * 100
            bah_annual_return = ((math.exp(math.log(max(money_bah/10000.0, 0.000001))/number_of_years)-1)*100)
            
            # Calculate strategy metrics
            strategy_return_pct = (stats.money / 10000.0 - 1) * 100
            strategy_annual_return = ((math.exp(math.log(max(stats.money/10000.0, 0.000001))/number_of_years)-1)*100)
            ...
            avg_trade_length = 0
            if stats.transaction_count > 0:
                success_rate = (stats.success_transaction_count / stats.transaction_count) * 100
                avg_profit_per_trade = (stats.total_percent_profit / stats.transaction_count) * 100
                avg_trade_length = stats.total_transaction_length / stats.transaction_count
            ...
            metrics = {
                ...
                "Avg_Trade_Length": round(avg_trade_length),
                "Max_Profit_per_Trade": round((stats.maximum_gain / max(stats.money, 1)) * 100, 2),
                "Max_Loss_per_Trade": round((stats.maximum_lost / max(stats.money, 1)) * 100, 2),
                "Maximum_Capital": round(stats.maximum_money, 2),
                "Minimum_Capital": round(stats.minimum_money, 2),
                "Idle_Ratio": round(((data_length - stats.total_transaction_length) / max(data_length, 1)) * 100, 2)
            }
            return metrics
