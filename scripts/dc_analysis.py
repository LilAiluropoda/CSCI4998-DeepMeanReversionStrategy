...
class dc_calculator():
	...
	def load_time_series_data_from_file(self, file: str, data_point_limit = None, kalman_filter: bool = False):
		...
		try:
			...
			data = pd.read_csv(file, header=None, names=["Date", "Open", "High", "Low", "Close", "Volume", "Adj Close"])
			data["Date"] = pd.to_datetime(data["Date"])
			data = data[data["Date"].dt.year == 2002]
			data = data.sort_values('Date').reset_index(drop=True)
			self.prices = data["Adj Close"].to_numpy()
...
if __name__ == '__main__':
	...
	calc.load_time_series_data_from_file("/home/LilAiluropoda/Projects/DeepMlpGA/app/data/stock_data/AAPL/AAPL19972007.csv")
