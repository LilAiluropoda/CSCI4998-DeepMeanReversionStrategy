...
def load_and_prepare_stock_data(
    ...
    Returns:
        tuple: (DataFrame with processed stock data, numpy array of close prices, numpy array of log returns)
    """
    try:
        ...
        # Read the CSV file
        df = pd.read_csv(file_path, header=None, names=columns, parse_dates=["Date"])
        ...
        return processed_df, close_prices, log_returns

    except Exception as e:
        raise Exception(f"Error loading stock data: {str(e)}")


def perform_stl_analysis(
    close_prices, period=22
):  # 22 for monthly seasonality (trading days)
    """
...
def sliding_window_rqa(
    data, window_size=260, step_size=22, embed_dim=11, delay=1, threshold=0.16
):
    n_points = len(data)
...
def main():
    ...
    _, close_prices, returns = load_and_prepare_stock_data(
        "/home/LilAiluropoda/Projects/fyp_project/app/data/stock_data/JPM/JPM19972007.csv"
    )
    ...
    rr_ret, det_ret, lam_ret, avg_len_ret, windows_ret = sliding_window_rqa(
        normalized_returns
    )
    rr_price, det_price, lam_price, avg_len_price, windows_price = sliding_window_rqa(
        normalized_price
    )
